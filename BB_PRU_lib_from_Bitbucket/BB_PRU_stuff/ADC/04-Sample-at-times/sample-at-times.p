// read adc ch 0 at times specified by C program.

// 1. CPU runs PRU code
// 2. pru code waits and counts, checking a reg until the CPU writes its clock time into it.
	// 3. CPU does clock_gettime and writes it in PRU data ram
	// 4. CPU writes a "good to go" in the reg that the PRU is watching.
// 5. PRU notes when "good to go" bit flipped, remembers its cycle time.
	// Now we have a decent connection between CPU time and PRU time.

// oh wait: maybe we can arrange this whole project so that we don't have to
	// map btwn CPU time and PRU time. Could we do everything in PRU time?
	// The MPC alg just cares about L past inputs and T future outputs.
	// Could there be a way for it to retrieve the last several ADC values
