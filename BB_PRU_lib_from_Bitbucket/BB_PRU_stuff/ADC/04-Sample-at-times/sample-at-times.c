...
int main (void)
{
  ...
  printf("Here's the cmd: root@beaglebone:/home/debian/beaglebone_pru_adc/src# echo EBB-PRU-Example > /sys/devices/bone_capemgr.9/slots\n");
   ...
   const char* binname =  "./sample-at-times.bin";
 ...
 printf("Waiting...\n"); 
   ...
   unsigned int num_samples =  *addr; 
   addr++;

   printf("# ADC samples to get: %u\n",num_samples);

   unsigned int start, adc, end;
   unsigned int i;
   for( i=0; i<num_samples; i++ ) {
     ...
     printf("start: %u\t\tADC: %u\t\tend: %u\t\tdelta (ns): %u\n",start,adc,end,(end-start)*5);

   }

   /* int diff = end-start; */
   /* printf("# cycles that ADC took to sample: %d (%d ns)\n",diff,5*diff); */
