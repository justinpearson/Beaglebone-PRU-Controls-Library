...
//         - write sample #, cycle count, adc, eqep, and gpio to data pkt in pru-sensor-buf.
	...
	ADD pru_data_buf_write_head, pru_data_buf_write_head, 4
	// duty
	SBBO duty_actual, tmp0, pru_data_buf_write_head, 4
	ADD pru_data_buf_write_head, pru_data_buf_write_head, 4
	// last time PRU heard a CPU data request
	SBBO time_of_last_cpu_data_req, tmp0, pru_data_buf_write_head, 4
	ADD pru_data_buf_write_head, pru_data_buf_write_head, 4
	// last time PRU rx'd a CPU cmd buffer
	SBBO time_of_last_cpu_cmd_sch, tmp0, pru_data_buf_write_head, 4
	ADD pru_data_buf_write_head, pru_data_buf_write_head, 4
...
//     - copy from CPU-sch-buf to PRU-sch-buf, then set 'ack-new-sch' flag
	...
	// Compare that to the current sample num (num_total_samples)
	// Throw out given cmd pkts whose sample times are in the past.
	// If num_total_samples 
	
	// For simplicity,
	// Totally overwrite our existing actuation schedule.
	// Fast-forward the idx_in_pru_cmd_buf until the sample time is the num_total_samples.
...
CHECK_FOR_OLD_CMD_PKTS:
	...
	qblt DONE_DISCARDING_STALE_CMD_PKTS, tmp2, num_total_samples
...
ACTUATE:
	...
	lbbo duty_cmd, tmp1, tmp0, 4 // note: duty goes into a 2-byte reg, so only the lower 2 bytes actually get used here.
