...
//         - write sample #, cycle count, adc, eqep, and gpio to data pkt in pru-sensor-buf.
	...
	// cycle count
	SBBO sample_time, rADDR_PRU_DATA_BUF, pru_data_buf_write_head, 4
...
INIT_REGISTERS:
	...
	mov tmp3 			, 0
	mov current_time 		, 0
	mov cycles_at_next_sample_time 	, DEFAULT_SAMPLE_PERIOD
	...
	mov pru_data_buf_write_head	, 0
	mov idx_in_pru_data_buf		, 0
	mov current_sample		, 1 // yes, 1, bc sample #1 happens at time 1*Ts, and covers time interval [0*Ts,1*Ts) in terms of actuation.
	...
	mov duty_cmd			, 333
	mov duty_actual			, 333 // 10% : period = 3333 ticks of 100MHz clock
	mov sample_period               , DEFAULT_SAMPLE_PERIOD
